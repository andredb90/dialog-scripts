...
apps=(
    "Final Configuration,/Library/Management/Enrollment/install_complete.txt"
    "Downloading FileSets,/Library/Management/Enrollment/download_complete.txt"
)
...
function filewave_dialog_progress {

tail -1 -f $filewave_log | while read line               
        do
                case "$line" in
                		*"Downloading Fileset"*)
                        echo "listitem: title: Downloading FileSets, statustext: $( echo ${line##*Fileset } | awk -F "(,)+" '{print $1}' ), status: wait" >> $dialog_command_file
                        ;;
						*"Done activating"*|*"Activate all"*)
						echo "progresstext: "${line##*|} | awk -F "(,)+" '{print $1}'"" >> $dialog_command_file
                        ;;
                        *"Running Installer"*)
...
while test $# -gt 0 ; do
    case "$1" in
        --filewave) filewave="yes"
            ;;
        --filewave-progress) filewaveprogress="yes"
            ;;
        --ms365) ms365="yes"
            ;;
    esac
...
if [[ $filewaveprogress == "yes" ]]; then
   echo "Start reading FileWave Client log..."
   filewave_dialog_progress & 
fi
